...
void main(void)
{
    ...
    Display_Panel_String(PANEL_LCD_LINE1, " SSer");

    while (1U)
    {
        /* Wait for any key press */
        if (g_switch_flag & SWITCHPRESS_ALL)
        {
            /* Start synchronous serial operation */
            Sync_Transfer();
            
            /* Reset the first array bytes to '0' or increment by 1 */
            if (ASCII_9 == g_csi00_txbuffer[0])
            {
                g_csi00_txbuffer[0] = ASCII_0;
                g_csi10_txbuffer[0] = ASCII_0;
            }
            else
            {
                g_csi00_txbuffer[0]++;
            ...
            g_switch_flag &= (uint8_t) ~SWITCHPRESS_ALL;
        }
    }
    /* End user code. Do not edit comment generated here */
}
